...
export class WordGroupProfileComponent {
  ...
  // gets the word group we are looking at out of the param map
  wordGroup = toSignal(
  ...
  words = toSignal(
    this.route.paramMap.pipe(
      ...
      switchMap((wordGroup: string) =>
        this.wordService.getWordsByWordGroup(wordGroup)
      ),
      catchError((_err) => {
        this.error.set({
  ...
  /**
   * Returns the number of words in a word group
   * @param nothing
   * @output integer
   */
  ...
  /**
   * Takes all words in the word group and applies pagination
   */
  ...
  /**
   * Updates values associated with page events sent by pagination
   * (changing the page size or page number)
   * @param $event
   */
