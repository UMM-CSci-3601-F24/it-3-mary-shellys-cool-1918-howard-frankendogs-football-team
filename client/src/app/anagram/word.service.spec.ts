...
describe('WordService', () => {
    ...
    it('calls api/anagram', waitForAsync(() => {
      ...
      wordService.getWords().subscribe(() => {
        ...
        expect(mockedMethod)
          ...
          .toHaveBeenCalledWith(wordService.wordUrl, {
  ...
  describe('When getWords() is called with parameters it forms proper HTTP request(Sever Filtering)', () => {
    ...
    it('correctly calls api/anagram with  filter parameter contains', () => {
      ...
      wordService.getWords({ word: 'c' }).subscribe(() => {
        ...
        expect(mockedMethod)
          .withContext('talks to correct endpoint')
          .toHaveBeenCalledWith(wordService.wordUrl, {
            params: new HttpParams().set('word', 'c'),
          });
      });
    });
    it('correctly calls api/anagram with filter parameter group', () => {
      ...
      wordService.getWords({ wordGroup: 'teach' }).subscribe(() => {
        ...
        expect(mockedMethod)
          .withContext('talks to correct endpoint')
          .toHaveBeenCalledWith(wordService.wordUrl, {
            params: new HttpParams().set('wordGroup', 'teach'),
          });
      });
    });
    it('correctly calls api/anagram with filter parameter filterType', () => {
      ...
      wordService.getWords({ filterType: 'exact' }).subscribe(() => {
        ...
        expect(mockedMethod)
          .withContext('talks to correct endpoint')
          .toHaveBeenCalledWith(wordService.wordUrl, {
            params: new HttpParams().set('filterType', 'exact'),
          });
      });
    });
    it('correctly calls api/anagram with more than one filter parameters', () => {
      ...
      wordService.getWords({ word: 'l', wordGroup: 'mem' }).subscribe(() => {
        ...
        expect(calledHttpParams.keys().length).toBe(2);
  ...
  describe('sorting on the client (alphabetical)', () => {
    it('returns the list in the correct way when sorting by word', () => {
      const filteredWords = wordService.sortWords(testWords, {
        sortType: 'alphabetical',
        sortOrder: false,
        sortByWordOrGroup: 'word',
      });
  ...
  describe('Deleting a word using deleteWord()', () => {
    it('Talks to correct endpoint with correct call', waitForAsync(() => {
      ...
      const mockedMethod = spyOn(httpClient, 'delete').and.returnValue(
        of(targetWord)
      );
      ...
      wordService.deleteWordGroup(targetGroup).subscribe(() => {
        ...
        expect(mockedMethod)
          ...
          .toHaveBeenCalledWith(
            `${wordService.wordUrl}/wordGroup/${targetGroup}`
          );
      });
    }));
  });
  ...
  describe('Word Group Profiles', () => {
    it('calls api/wordGroup/{id} where id is the target word group', waitForAsync(() => {
      const targetGroup = 'team member';
      const mockedMethod = spyOn(httpClient, 'get').and.returnValue(
        of(testWords.slice(1, 3))
      );
